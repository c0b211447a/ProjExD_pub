...
def main():
    ...
    screen = Screen('ProjExD_pub/fig/pg_bg.jpg', (1600, 900)) #画面用のSurfaceを保持する変数screen
    ...
    tori.add(character1('ProjExD_pub/fig/3.png', 2.0, (200, 450),distance%60)) #toriグループにキャラクターのspriteを追加する
    ...
    while True:
        ...
        if knife_flg: #knife_flgがTrueなら
            if count % 250 == 0: #countが250進むごとに
                knife1.add(knife_under('ProjExD_pub/fig/enemy.jpg', (2000, 900), y_kulist, index_num)) #knife1グループに下側ナイフのspriteを追加する
                knife2.add(knife_top('ProjExD_pub/fig/enemy.jpg', (2000, 0), y_ktlist, index_num)) #knife2グループに上側ナイフのspriteを追加する
        ...
        if score_flg: #score_flgがTrueなら
            if pg.sprite.groupcollide(knife1, tori, True, True) or pg.sprite.groupcollide(knife2, tori, True, True):
                ...
                #もしknife1グループとtoriグループもしくはknife2グループとtoriグループが接触していたら
                #両グループの接触しているspriteをGroupから削除して
                chr_count -= 1 #キャラクターのライフを1だけ減らす

                if chr_count == 2: #キャラクターのライフが2のとき
                    tori.add(character1('ProjExD_pub/fig/4.png', 0.5, (200, 450),distance%60)) #あらたに'ProjExD_pub/fig/4.png'の画像をtoriグループに加える
                if chr_count == 1: #キャラクターのライフが1のとき
                    tori.add(character1('ProjExD_pub/fig/5.png', 0.20, (200, 450))) #あらたに'ProjExD_pub/fig/5.png'の画像をtoriグループに加える
            ...
            if chr_count == 0: #キャラクターのライフが0のとき
                ...
                screen.disp.blit(txt, (1600/2-250, 900/2-75)) #txtを位置(1600/2-250, 900/2-25)に描画する
